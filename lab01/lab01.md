...
2. Какие языки (если есть) ваш браузер может принимать? В захваченном сеансе какую еще
   ...
   - Браузер может принимать только английский язык
   - Версию ОС хоста (Windows NT 10.0; Win64; x64)
3. Какой IP-адрес вашего компьютера? Какой адрес сервера gaia.cs.umass.edu?
   - Компьютер - 172.20.10.3
...
5. Когда HTML-файл, который вы извлекаете, последний раз модифицировался на сервере?
   - 22 февраля 06:59:01 GMT
6. Сколько байтов контента возвращается вашему браузеру?
   - 540
![image](https://github.com/bobr-iz-ozera/networks-course/assets/54845221/fe69cb51-b79c-4a2a-92be-4960f0c502af)
...
2. Проверьте содержимое ответа сервера. Вернул ли сервер содержимое файла явно? Как вы
   это можете увидеть?
   - Да
   - Код 200, информация о файле и строка "content length"
3. Теперь проверьте содержимое второго HTTP-запроса GET (из вашего браузера на сторону
   ...
   - Дата последней модификации файла (22 февраля 06:59:01 GMT)
4. Какой код состояния HTTP и фраза возвращаются сервером в ответ на этот второй запрос
   ...
   - Код 294, Not modified
   - Нет не вернул, тк нет строки с информацией о размере полученного файла
![image](https://github.com/bobr-iz-ozera/networks-course/assets/54845221/003894d7-706b-4af4-a6d1-884fc259c1ac)
...
4. Есть ли в передаваемых данных какая-либо информация заголовка HTTP, связанная с
   сегментацией TCP?
   - Ничего кроме вкладки о 4ёх фрагментах я не нашёл
![image](https://github.com/bobr-iz-ozera/networks-course/assets/54845221/46148c43-79b3-4438-a57e-04f36d0b8a1c)
...
1. Сколько HTTP GET запросов было отправлено вашим браузером? На какие интернет-адреса были отправлены эти GET-запросы?
   - 3
   - gaia.cs.umass.edu (x2) и kurose.cslash.net
2. Можете ли вы сказать, загрузил ли ваш браузер два изображения последовательно или
   ...
   - 1) то как отображал прогрузку изображений браузер (они появлялись параллельно) + в wireshark в трекере пакетов были сначала 2 запроса get на картинки, потом 2 ответа с ними
